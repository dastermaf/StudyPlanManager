...
app.get('/api/progress', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length > 0) {
            console.log(`LOG: server.js: Найден прогресс для пользователя ${req.user.username}`);
...
app.post('/api/progress', authenticateToken, async (req, res) => {
    console.log(`LOG: server.js: Получен запрос на сохранение прогресса от пользователя ${req.user.username}`);
    try {
